...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    public void init() {
        log.info(ModuleType.AUTHORIZATION_MODULE + "【authorization-module自定义过滤器-JwtAuthenticationTokenFilter】已经加入容器中。");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (!StringUtils.hasText(token)) {
            log.info(ModuleType.AUTHORIZATION_MODULE + "请求[没有]携带的token,需要经过后续过滤器校验...");
        ...
        log.info(ModuleType.AUTHORIZATION_MODULE + "token解析出用户名" + userName + "。");

        // 从redis中获取用户信息
        CustomUserUserDetails user = redisUtil.get(userName, CustomUserUserDetails.class);
        if (Objects.isNull(user)) {
            log.info(ModuleType.AUTHORIZATION_MODULE + "[没有]在redis中找到用户名" + userName + "的缓存信息");
            throw new RRException("登录信息过期!");
        }
        log.info(ModuleType.AUTHORIZATION_MODULE + "【authorization-module】请求携带的token,解析出用户名【{}】，其缓存信息:{}", user.getUsername(), user.getAuthorities());
